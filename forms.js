...
(function () {
  ...
  const COUNTRY_DIAL_CODES = [
    ['UG', '+256'], ['AF', '+93'], ['AL', '+355'], ['DZ', '+213'], ['AS', '+1684'],
    ['AD', '+376'], ['AO', '+244'], ['AI', '+1264'], ['AG', '+1268'], ['AR', '+54'],
    ...
    ['GD', '+1473'], ['GP', '+590'], ['GU', '+1671'], ['GT', '+502'], ['GG', '+441481'],
    ['GN', '+224'], ['GW', '+245'], ['GY', '+592'], ['HT', '+509'], ['HN', '+504'],
    ['HK', '+852'], ['HU', '+36'], ['IS', '+354'], ['IN', '+91'], ['ID', '+62'],
    ['IR', '+98'], ['IQ', '+964'], ['IE', '+353'], ['IM', '+441624'], ['IL', '+972'],
    ['IT', '+39'], ['JM', '+1876'], ['JP', '+81'], ['JE', '+441534'], ['JO', '+962'],
    ['KZ', '+7'], ['KE', '+254'], ['KI', '+686'], ['XK', '+383'], ['KW', '+965'],
    ...
    ['NG', '+234'], ['NU', '+683'], ['NF', '+672'], ['KP', '+850'], ['MK', '+389'],
    ['MP', '+1670'], ['NO', '+47'], ['OM', '+968'], ['PK', '+92'], ['PW', '+680'],
    ['PS', '+970'], ['PA', '+507'], ['PG', '+675'], ['PY', '+595'], ['PE', '+51'],
    ['PH', '+63'], ['PL', '+48'], ['PT', '+351'], ['PR', '+1787'], ['QA', '+974'],
    ['RE', '+262'], ['RO', '+40'], ['RU', '+7'], ['RW', '+250'], ['BL', '+590'],
    ['SH', '+290'], ['KN', '+1869'], ['LC', '+1758'], ['MF', '+590'], ['PM', '+508'],
  ...
  function enhanceCountryCodeSelects() {
    const selects = document.querySelectorAll(
      '#event-country-code, #donor-country-code, #signup-country-code, #reg-country-code, #mem-country-code'
    );

    selects.forEach((select) => {
      const currentValue = select.value || '+256';
      const currentCountry = select.selectedOptions[0]?.dataset.country || 'UG';

      select.innerHTML = '';

      COUNTRY_DIAL_CODES.forEach(([country, dial]) => {
        const option = document.createElement('option');
        option.value = dial;
        option.dataset.country = country;
        option.textContent = `${flagEmoji(country)} ${country} ${dial}`;
        option.selected = dial === currentValue && country === currentCountry;
        select.appendChild(option);
      });

      if (!select.value) {
        const fallback = Array.from(select.options).find((option) => option.value === currentValue);
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', enhanceCountryCodeSelects);
  } else {
    enhanceCountryCodeSelects();
  }
})();
